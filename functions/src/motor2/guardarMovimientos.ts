...
export const guardarMovimientos = onRequest(
  ...
  async (req, res) => {
    ...
    const periodoRef = db.collection("empresas").doc(empresa).collection("periodos").doc(periodo);
    ...
    let pending: Array<{ ref: admin.firestore.DocumentReference; data: Record<string, unknown> }> = [];
    let opsInBatch = 0;
