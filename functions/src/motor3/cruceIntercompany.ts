...
export const cruzar = onRequest(
    ...
    async (req, res) => {
        ...
        const tolerancia = Number.isFinite(toleranciaParsed)
            ? Math.max(0, toleranciaParsed)
            : 0;
        ...
        try {
            ...
            for (const empresaDoc of empresasSnap.docs) {
                const movimientosSnap = await empresaDoc.ref
                    .collection("periodos")
                    .doc(periodo)
                ...
                for (const doc of movimientosSnap.docs) {
                    ...
                    const empresaOrigen = normalizeId(
                        String(data.empresa_origen ?? ""),
                    );
                    const empresaContraparte = normalizeId(
                        String(data.empresa_contraparte ?? ""),
                    );
                    ...
                    if (tipo === "CxC") {
                        sumInto(
                            mapaCxC,
                            `${empresaOrigen}|${empresaContraparte}`,
                            valor,
                        );
                    } else {
                        sumInto(
                            mapaCxP,
                            `${empresaContraparte}|${empresaOrigen}`,
                            valor,
                        );
                    }
                }
            ...
            for (const clave of claves) {
                ...
                const estado: CruzarResultado["estado"] =
                    Math.abs(diferencia) <= tolerancia ? "OK" : "DESCUADRE";
                if (estado === "OK") ok++;
                else descuadre++;
            ...
            logger.error("MOTOR 3 ERROR", error);
            res.status(500).json({
                error: "MOTOR_3_ERROR",
